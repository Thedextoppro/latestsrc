...
export function useGasPrice(): string {
  ...
  const { data } = useFeeData({
    chainId,
    enabled: chainId !== ChainId.PULSE_CHAIN && chainId !== ChainId.BSC_TESTNET,
    watch: true,
  })
  if (chainId === ChainId.PULSE_CHAIN) {
    return userGas
  }
...
export function toV2LiquidityToken([tokenA, tokenB]: [Token, Token]): Token {
  return new Token(tokenA.chainId, Pair.getAddress(tokenA, tokenB), 18, 'DLP', 'DexTop LP')
}
