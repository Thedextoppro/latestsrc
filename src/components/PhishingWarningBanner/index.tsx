...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    const warningText = t("please make sure you're visiting https://dextop.pro - check the URL carefully.")
    return warningText.split(/(https:\/\/dextop.pro)/g)
  }, [t])
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://dextop.pro'}
          color={text === 'https://dextop.pro' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
