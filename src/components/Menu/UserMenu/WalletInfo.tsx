...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowNativeBalance, onDismiss }) => {
  ...
  const isBSC = chainId === ChainId.PULSE_CHAIN
  const bnbBalance = useBalance({ addressOrName: account, chainId: ChainId.PULSE_CHAIN })
      ...
      <Box mb="24px">
        <Flex justifyContent="space-between" alignItems="center" mb="8px">
          <Flex bg={COLORS.BNB} borderRadius="16px" pl="4px" pr="8px" py="2px">
            <ChainLogo chainId={ChainId.PULSE_CHAIN} />
            <Text color="white" ml="4px">
              Pulse Chain
            </Text>
          </Flex>
          <LinkExternal href={getBlockExploreLink(account, 'address', ChainId.PULSE_CHAIN)}>
            {getBlockExploreName(ChainId.PULSE_CHAIN)}
          </LinkExternal>
        </Flex>
        <Flex alignItems="center" justifyContent="space-between">
          <Text color="textSubtle">Pulse {t('Balance')}</Text>
        ...
        <Flex alignItems="center" justifyContent="space-between">
          <Text color="textSubtle">{t('DEX Balance')}</Text>
